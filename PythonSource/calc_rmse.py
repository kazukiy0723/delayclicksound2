...
# CSVファイルの読み込み
# filename = '.\\Result_SoundAndLight\\result_yamashita_bpm90.csv'  
# filename = '..\\result717\\result717.csv'
filename = r'G:\マイドライブ\temp\RESULT\result717\result717.csv'
df = pd.read_csv(filename, header=None)

# 3列目から最後から2列目までを選択し、各行の標準偏差を計算
df_selected = df.iloc[:, 3:-2]

# 指定した真の値
true_values = 60 / 90 * 1000  
...
width  = 0.5 / 2  # 幅を2で割ることで、2つの棒が重ならないようにする
...
if(len(x_labels) != len(rmse_values)):
    ...
    plt.title('BPM90 Yamashita Sound and Light')                   # タイトルは適切なものに変更する
